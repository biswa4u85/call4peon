...
//        pr($data);exit;
    ...
    function getResult(){ 
//        $query = "SELECT admin.iAdminId,log_history.dLogoutDate
//                FROM admin
//                Join log_history
//                ON admin.iAdminId = log_history.iUserId 
//                WHERE admin.iAdminId = 1 
//                ORDER BY log_history.dLogoutDate DESC LIMIT 1";
//        echo $query;exit;
        $adminid = $this->session->userdata('iAdminId');
        
        $this->db->select('admin.*,log_history.dLoginDate');
        $this->db->from('admin');
        $this->db->join('log_history',"admin.iAdminId = log_history.iUserId");
        $this->db->where("admin.iAdminId",$adminid);
