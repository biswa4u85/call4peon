...
//         echo $this->db->last_query();exit;
    ...
    function getState($fields = '', $tbl = '', $con) {
        if ($con != '') {
            $this->db->select($fields);
            if ($tbl != '') {
                $this->db->from($tbl);
...
//            print_r($this->db->last_query());exit;
    ...
    function getCity($fields = '', $tbl = '', $con, $con1) {
        if ($con != '') {
            $this->db->select($fields);
            if ($tbl != '') {
                $this->db->from($tbl);
    ...
    function getOwnerList($id = '') {
        ...
        $this->db->where("user_master.iCompanyId", $this->config->item('YT_USER_COMPANY_ID'));
    ...
    function checkDataExist($tbl) {
        if ($tbl == "activities") {
            ...
            if (strtolower($this->config->item('YT_USER_PROFILE')) == "administration") {
                $this->db->where("iCompanyId", $this->config->item('YT_USER_COMPANY_ID'));
            } else {
            ...
            if (strtolower($this->config->item('YT_USER_PROFILE')) == "administration" && $tbl != "activities") {
                $this->db->where("iCompanyId", $this->config->item('YT_USER_COMPANY_ID'));
    ...
    function insertActivity($data = array()) {
        ...
        $data['iCompanyId'] = $this->config->item('YT_USER_COMPANY_ID');
...
//            pr($reply);exit;
    ...
    function checkUserRole($roleId) {
        ...
        $this->db->from("user_master");
        $this->db->where("iUserRoleId", $roleId);
        $this->db->where("isDelete != ", 1);
        $result = $this->db->get()->result_array();
    ...
    public function get_contain ($pagecode) {
            $this->db->select('*');
            $this->db->from('page_settings');
