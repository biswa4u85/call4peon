...
function gettodo() {
    $.ajax({
	type: 'POST',
	url: rootPath + 'content/todo',
	success: function (data) {
	    $('#todolist').html(data);

	    $('[name=todolsit]').click(function () {
		$.ajax({
		    type: 'POST',
		    url: rootPath + 'content/uptodo',
		    data: {module: $(this).data('type'), id: $(this).data('id'), type: 'ajax'},
		    success: function (data) {
...
function setgrid(html_id, table, editpath, Colsmodel, Colshead, Colstitle, options, datatypes, tblheads, listorder, order_list, show_order_list) {
	...
	$('.productdelete').unbind('click');
	$(document).off('click', '.productdelete');

	$(document).on('click', '.productdelete', function (e) {
	    var productid = (typeof ($(this).data('id')) !== 'undefined') ? $(this).data('id') : '';
	    var producttype = (typeof ($(this).data('type')) !== 'undefined') ? $(this).data('type') : '';

	    var confirmmsg = 'The ' + producttype + ' you have chosen to delete may have various records associated to it. Any record(s) associated to this ' + producttype + ' will also be deleted.';

	    if (confirm(confirmmsg)) {
		$.ajax({
		    type: 'POST',
		    async: false,
		    url: rootPath + 'products/delete_product',
			...
			if (result == "true") {
				...
				alert("You cannot delete this " + producttype + " because its data is associated in Hotel/Package/Quotation");
			    }
			    else {
				alert("You cannot delete this " + producttype + " because its data is associated in Package/Quotation");
			    }
			}
		    }
		});
	...
	$('#' + html_id + ' tbody').on('click', 'td', function (e) {
		...
		if ($('#' + html_id).data('type') == 'attach') {
		    $(this).parent().find('a.listviewbtn').trigger('click')
		} else {
		    var viewurlval = $(this).parent().find('a.listviewbtn').attr('href');
...
function onloadbody() {
    ...
    $.ajax({
	type: 'POST',
	url: rootPath + 'feeds/getNotifications',
	success: function (data) {
	    $('#notification_panel').append(data);
    ...
    $.ajax({
	type: 'POST',
	url: rootPath + 'feeds/getNotificationCount',
	success: function (data) {
	    if (data == '0') {
		$('#noti_count').remove();
	    }
	    else {
		$('#noti_count').addClass('alert_notification red_alert');
		$('#noti_count').empty();
		$('#noti_count').append(data);
	    }
	}
    });
}
