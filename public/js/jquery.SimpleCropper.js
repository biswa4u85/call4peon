...
(function ($) {
    ...
    $.fn.simpleCropper = function () {
        ...
        var image_dimension_x = 200;
        var image_dimension_y = 200;
        var scaled_width = 0;
        var scaled_height = 0;
        var x1 = 0;
        var y1 = 0;
        ...
        function preview() {
            ...
            var selector = file_display_area.find('.file_image');
            var selector1 = file_display_area.next('.deleteImage');
            
            var imageFoo = document.createElement('img');
            
            $.ajax({
                type: "POST",
                url: rootPath + "content/saveImage",
                data: {
                ...
                if (response !== "failed") {
                    ...
                    if (selector.data('div') === "profile") {
                        $(".user_profile_img").attr('src', value);
                    }
                }
                else {
                    alert("Problem in image upload.");
                }
            });
