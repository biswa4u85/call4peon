...
(function (window) {
    ...
    function getTemplate(el, options) {
        // Template html
	var html = '<div class="box-editor">' +
		...
		' </div>' +
		'<div id="textBox' + options.textArea + '" contenteditable="true" class="txtbox">' +
		'</div></div></div>';
	// render html
        el.innerHTML = html;
    }

    function initDoc(fm, bx, text) {
	var elem = bx,
		sDefTxt;
	if (text !== "") {
	...
	_init: function () {
	    var self = this;
	    // Div of content
	    this.box = document.querySelector('#textBox' + this.options.textArea);
	    ...
	    // Init Doc function
	    initDoc(this, this.textArea, this.text);
